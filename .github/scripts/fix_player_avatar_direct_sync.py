...
if "syncPlayerAvatarToSupabase(player, file, previousAvatar)" not in text:
    if text.count(old) != 1:
...
c = cfg.read_text()
if "VOLLEY_ASSET_VERSION = '20260812t'" not in c:
...
idx = Path('index.html')
i = idx.read_text()
if 'app.js?v=20260812t' not in i:
