...
// Guardado rápido y único de Pasar Lista.
// Usa los UUID ya hidratados en la plantilla y evita consultas por jugadora.
...
async function eventUuid(value){
  ...
  const direct=local?.supabaseId||local?.supabase_id;
  if(direct&&UUID.test(String(direct)))return String(direct);
  const legacy=local?.legacy_id||local?.legacyId||id;
  const {data,error}=await c.from('events').select('id').eq('legacy_id',String(legacy)).maybeSingle();
  if(error)throw error;
  return data?.id||null;
}

async function ensureMissingPlayerIds(players){
  const missing=players.filter(p=>!UUID.test(String(p.supabaseId||'')));
...
async function handleSubmit(event){
  ...
  try{
    ...
    const coachCandidate=user?.authId||user?.id||null;
    const coachId=UUID.test(String(coachCandidate||''))?coachCandidate:null;
    ...
    for(const player of state.players||[]){
      ...
      if(!cb)continue;
      const playerId=player.supabaseId;
      if(!UUID.test(String(playerId||'')))continue;
      const officialStatus=cb.checked?'present':'unjustified';
      if(cb.checked)present++;
      rows.push({event_id:eid,player_id:playerId,official_status:officialStatus,validated_by:coachId,validated_at:now,updated_at:now});
    }
    ...
    // Feedback inmediato. La rehidratación completa ocurre después y no bloquea el botón.
    ...
    setTimeout(async()=>{
      try{
        if(typeof window.loadAttendanceFromSupabase==='function')await window.loadAttendanceFromSupabase({silent:true,force:true});
        if(typeof renderCoachAttendanceList==='function')renderCoachAttendanceList();
        if(typeof renderHomeDashboard==='function')renderHomeDashboard();
      }catch(error){console.warn('[AttendanceBatchSave] post-refresh',error);}
    },0);
  }catch(error){
...
console.info('[AttendanceBatchSave] Guardado rápido en lote activado.');
})();