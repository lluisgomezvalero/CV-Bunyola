...
const currentUser=()=>{try{return typeof getCurrentUser==='function'?getCurrentUser():null}catch(_){return null}};
  ...
  try{
    ...
    for(const row of data||[]){
      ...
      if(row.player_response==='yes'||row.player_response==='no'){
        confirmations.push({
          id:row.id,eventId:ev.id,eventIdLegacy:ev.legacy_id||ev.legacyId||null,
          playerId:p.id,playerIdLegacy:p.legacy_id||p.legacyId||null,
          status:row.player_response,timestamp:row.updated_at||row.created_at,
          supabaseEventId:row.event_id,supabasePlayerId:row.player_id
        });
      }
      if(row.official_status){
        official.push({
          id:row.id,eventId:ev.id,eventIdLegacy:ev.legacy_id||ev.legacyId||null,
          playerId:p.id,playerIdLegacy:p.legacy_id||p.legacyId||null,
          playerName:p.name||p.username||'Jugadora',status:row.official_status,
          source:'supabase',validatedAt:row.validated_at||row.updated_at||row.created_at,
          supabaseEventId:row.event_id,supabasePlayerId:row.player_id
        });
      }
    }
...
async function confirmAttendance(eventId,status,btn){
  ...
  try{
    ...
    let error=null;
    if(existing?.id){
      ({error}=await c.from('attendance').update({player_response:status,updated_at:new Date().toISOString()}).eq('id',existing.id));
    }else{
      ({error}=await c.from('attendance').insert({event_id:eid,player_id:pid,player_response:status}));
    }
...
function drawRollCall(eventId){
  ...
  container.innerHTML='';
  const confirmations=st.trainingConfirmations||[]; const official=st.attendanceData||[];
...
async function saveRollCall(event){
  ...
  savingRollCall=true;
  const form=event.target; const btn=form.querySelector('button[type="submit"]');
  if(btn){btn.disabled=true;btn.textContent='Guardando…';}
  try{
    const st=state(),c=db(); const localEventId=document.getElementById('verify-attendance-event-id')?.value;
    ...
    for(const p of st?.players||[]){
      ...
      const remote=[...pmap.entries()].find(([,lp])=>lp===p)?.[0]||p.supabaseId;
      if(!remote) continue;
      const status=cb.checked?'present':'unjustified'; if(cb.checked)present++;
      const {data:existing,error:ferr}=await c.from('attendance').select('id').eq('event_id',eid).eq('player_id',remote).maybeSingle();
      if(ferr) throw ferr;
      const payload={official_status:status,validated_at:new Date().toISOString()};
      let error=null;
    ...
    savingRollCall=false; if(btn){btn.disabled=false;btn.textContent='Confirmar Lista';}
  }
}

// Este listener se registra ANTES que los parches antiguos y neutraliza su submit de Pasar Lista.
...
function installOverrides(){
  ...
  const wait=()=>{
    ...
    // Se aplican deliberadamente al final para sustituir TODAS las capas antiguas de asistencia.
