...
function attackEditor(){
  ...
  return `<section class="gp2-section"><div class="gp2-section-head"><span>1</span><div><h3>Tendencias de ataque rival</h3><p>Configura cada atacante por separado.</p></div></div>
    ...
    <article class="gp2-attack-card">
      <header><div><small>${esc(meta.role)}</small><input data-gp2-attacker-name="${key}" value="${esc(a.name)}" placeholder="Nombre o dorsal"></div><label><input type="checkbox" data-gp2-attacker-visible="${key}" ${a.visibleToPlayers?'checked':''}><span>Visible</span></label></header>
...
function ourServeEditor(){
  ...
  const known=opts.find(([k,v])=>target===v||target===k);
  return `<section class="gp2-section"><div class="gp2-section-head"><span>3</span><div><h3>Nuestro saque</h3><p>Define a quién buscamos y las zonas objetivo.</p></div></div>
    <div class="gp2-target-tabs">${opts.map(([k,v])=>`<button type="button" data-gp2-target="${esc(v)}" class="${target===v||target===k?'active':''}">${k}</button>`).join('')}<button type="button" data-gp2-target-other class="${target&&!known?'active':''}">Otro</button></div>
    <input class="gp2-target-input" data-gp2-target-input value="${esc(target&&!known?target:'')}" placeholder="Nombre o dorsal" ${target&&!known?'':'hidden'}>
...
function handleChange(target){
  ...
  if(target.hasAttribute('data-gp2-target-input')){ui.draft.servePlayerTarget=String(target.value||'').trim();setDirty();return true;}
...
function bindEvents(){
  ...
  document.addEventListener('click',event=>{
    ...
    if(t.closest?.('[data-gp2-target-other]')&&coach()){ui.draft.servePlayerTarget='';setDirty();renderCoach();setTimeout(()=>document.querySelector('[data-gp2-target-input]')?.focus(),0);return;}
