(function () {
  ...
  async function syncWellness(){
    ...
    try{
      const {data,error}=await c.from('wellness_entries').select('id,player_id,entry_date,general_state,fatigue,soreness,stress,sleep,sleep_hours,notes,created_at,updated_at,players(id,legacy_id,profile_id)');
